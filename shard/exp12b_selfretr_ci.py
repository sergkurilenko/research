...
def shard_top1(Xrot, Qroute, Qscore, d_pub, kc):
    U = np.ascontiguousarray(Xrot[:, :d_pub]); Uq = np.ascontiguousarray(Qroute[:, :d_pub])
    short = S.topk_search(U, Uq, kc)
    G = Xrot[short]
    sc = np.einsum("qkd,qd->qk", G, Qscore)
    return short[np.arange(len(Qscore)), sc.argmax(1)]          # per-query reranked top-1 doc id
