...
def run(enc, lang):
    ...
    legacy_svd = per_query_ndcg(
        top_order(np.ascontiguousarray(Qrot_legacy[:, :k]) @ Pk.T), cids, qids, qrels)
    ...
    for pf in PUB_FRACS:
        ...
        U = np.ascontiguousarray(Drot[:, :dp]); Uq = np.ascontiguousarray(Qrot_legacy[:, :dp])
