...
def exp2() -> None:
    ...
    ax.boxplot(data, tick_labels=["ct-ct", "ct-pt"], patch_artist=True)
...
def exp6() -> None:
    # Canonical operating point k=192. The server-side ct-pt rerank is the only
    # k-dependent stage, so it is taken from the 5-seed integral run (exp6, k=192).
    # The client-side and HTTP stages are k-independent (they depend on M, the
    # CKKS poly degree N, K_cands and payload sizes, not on k), so they are taken
    # from the client-server HTTP-loopback run.
    agg = json.loads((NB / "exp6_outputs" / "exp6_aggregated.json").read_text(encoding="utf-8"))
    server_p95 = agg["p95_ms"]["mean"]
    cs = json.loads(
        (NB / "exp_cs_v2_outputs" / "cs_v2_results.json").read_text(encoding="utf-8")
    )["latency_pct_pooled"]
    http_p95 = cs["network_ms"]["p95"] - cs["server_ckks_ms"]["p95"]
    stages = [
        ("Local PQ", cs["client_pq_ms"]["p95"]),
        ("Encrypt", cs["encrypt_ms"]["p95"]),
        ("Server rerank", server_p95),
        ("HTTP+serial.", http_p95),
        ("Decrypt", cs["decrypt_ms"]["p95"]),
    ]
    fig, ax = plt.subplots(figsize=(5.0, 2.8))
    ax.bar([s[0] for s in stages], [s[1] for s in stages], color="#567a9f")
    ax.set_ylabel("p95 latency, ms (k=192)")
    ...
    ax.set_xlabel("Server-side rerank latency, ms (k=192)")
    ax.set_ylabel("CDF")
    ax.legend(frameon=False)
