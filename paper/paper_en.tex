...
\graphicspath{{figs/}}
\emergencystretch=3em  % absorb small overfull \hbox (long texttt model paths, URLs)
...
(i)~the \textsc{Shard} cell-keyed residual-splitting construction---the
first \emph{cancellable-template} scheme (revocable, renewable and
unlinkable at zero utility cost) for dense text retrieval
(Section~\ref{sec:shard});
(ii)~the $\approx\!C\times$ anchor-complexity scaling law and the resulting
bandwidth-optimal cell count $C^\star$ against a budgeted adversary, with
semi-formal accounts (Propositions~\ref{prop:anchor} and~\ref{prop:optC},
Sections~\ref{ssec:shard-align},~\ref{ssec:shard-cost});
...
\paragraph{\textsc{Shard} as a cancellable template scheme.} The keyed
residual shard $z_i=H_{c(i)}r_i$ is, in the language of biometric template
protection~\cite{ref_cancellable}, a \emph{cancellable template} for a text
embedding. It is \emph{(i)~non-invertible} without the cell key
$H_{c(i)}$---an unknown element of $\mathrm{O}(d_{\mathrm{priv}})$ carrying
$\binom{d_{\mathrm{priv}}}{2}$ secret angles; \emph{(ii)~revocable and
renewable}---a compromised store is re-issued by drawing fresh keys
$\{H_c'\}$, which re-randomises every shard while leaving the plaintext
similarity, and hence retrieval quality, \emph{exactly} unchanged
(Proposition~\ref{prop:exact}); and \emph{(iii)~unlinkable}---two stores of
the same corpus under independent keys are unrelatable in the residual
subspace without a key. To our knowledge this is the first transfer of the
cancellable-biometrics design pattern to dense text retrieval, and it is
what separates \textsc{Shard} from one-shot obfuscation: a leak is repaired
by \emph{re-keying}, at zero utility cost, rather than being permanent.
Because the keys are \emph{per cell}, the template is cancellable at the
granularity of the cell partition---the parameter $C$ that the next sections
turn into a security--bandwidth dial.
...
retrieval-trained $d\geq768$ encoders. Because \textsc{Shard} reranks
\emph{exactly} in the full space (Section~\ref{sec:shard}), it inherits
this projected-baseline utility unchanged; its own retrieval quality is
evaluated in Section~\ref{ssec:shard-utility}.
...
\paragraph{Choosing $C$: a security--bandwidth game.}
Proposition~\ref{prop:anchor} below and Table~\ref{tab:shard-cost} make the
cell count a one-parameter optimisation against a budgeted adversary. Fix a
threat model that grants the attacker at most $m_{\mathrm{adv}}$ known
plaintext--shard pairs, and let $\beta(C)$ be the expected active cells per
query (the upload cost, non-decreasing in $C$). The defender commits to $C$;
the attacker then allocates its anchors.
\begin{proposition}[Bandwidth-optimal cell count]\label{prop:optC}
Against a \emph{diffuse} adversary, \textsc{Shard} resists known-plaintext
alignment iff the anchor complexity exceeds the budget,
$C\,d_{\mathrm{priv}}/(1+\delta)>m_{\mathrm{adv}}$
(Proposition~\ref{prop:anchor}). Since $\beta$ is non-decreasing, the
bandwidth-minimal secure choice is
\[
C^\star=\big\lceil (1+\delta)\,m_{\mathrm{adv}}/d_{\mathrm{priv}}\big\rceil ,
\]
deployable iff $\beta(C^\star)\leq B$ for a bandwidth budget $B$. No finite
$C$ defends against a \emph{targeted} adversary, whose complexity
$d_{\mathrm{priv}}$ is independent of $C$; there the only recourse is the
micro-key limit $C{=}N$ with key rotation (Section~\ref{ssec:shard-micro}).
\end{proposition}
\noindent The frontier is favourable. Anchor complexity is \emph{linear} in
$C$ (Proposition~\ref{prop:anchor}) while the cost $\beta(C)$ grows
\emph{sub-linearly}---a query's neighbourhood spans only a bounded number of
ever-finer cells---so on the measured grid quadrupling $C$ from $64$ to
$256$ multiplies anchor complexity $4\times$ (e5-small $m_{50}:25.6\text{k}
\to102.4\text{k}$, Table~\ref{tab:shard-align}) but upload only
$\approx1.9\times$ ($15.7\to30.1$ active cells at $K_{\mathrm{cands}}{=}200$,
Table~\ref{tab:shard-cost}). Security-per-unit-bandwidth therefore
\emph{improves} with $C$, up to the micro-key endpoint.
...
universal privacy primitive. \emph{Calibrated Gaussian-noise diagnostic}:
