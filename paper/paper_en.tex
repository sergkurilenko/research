...
\begin{abstract}
\noindent
Dense embeddings underpin semantic search and Retrieval-Augmented Generation,
yet a leaked vector store hands much of the underlying text back to whoever
holds it. The modern attacks that make this possible---few-shot alignment,
zero-shot inversion, unsupervised cross-space translation---all turn on the
same weakness: the protected store is a \emph{single global geometry}, and any
single geometry can be aligned to a known one. A secret global rotation, the
usual lightweight defence, is no exception, since orthogonal Procrustes
recovers it once the attacker has assembled on the order of the subspace
dimension in known plaintext pairs.

We introduce \textsc{Shard}, a retrieval-preserving embedding transform built
to remove that weak axis rather than to mask a distortion surrogate. The
centred embedding is rotated and split into a short \emph{public} prefix,
which drives coarse stage-1 retrieval, and a \emph{private} residual, which is
sharded into $C$ cells and rotated cell by cell under separate secret keys;
the residual is reranked under CKKS, where those keys cancel and leave the
inner product exact. A single parameter $C$ carries the design from the
global-linear baseline it replaces ($C{=}1$) to per-document micro-keys
($C{=}N$). Measured on cached embeddings from five encoders, the transform
earns its keep on three fronts. Because the rerank is full-dimensional it
returns the raw-space nDCG@10 that half-SVD truncation gives up; because the
residual is keyed cell-locally, recovering it into a common frame under a
\emph{diffuse} known-plaintext leak costs roughly $C$ times more anchors (the
median rises from $200$ to $102{,}400$ at $C{=}256$) for only a handful of
encrypted residual queries per search; and because the public channel is a
short prefix rather than half the space, it exposes far less neighbour
structure, with a micro-key limit that drives the residual graph to zero and
leaves an unlinkable, renewable template. The barrier holds against the
stronger aligners we tried---learned-linear, non-linear, and
unsupervised---and where a distortion-aware noise defence at matched utility
still de-anonymises almost every probe, \textsc{Shard} de-anonymises none. We
are equally plain about what it does not do: within a cell the keys cancel and
similarities survive, a \emph{targeted} attacker who concentrates on one
victim's cell needs only about $d_{\mathrm{priv}}$ anchors, and an overlapping
reference corpus still leaks through the public prefix. \textsc{Shard} is an
...
orthogonal key $H_c$, a product of Householder reflections, so that the server
ever only stores $z_i = H_{c(i)} r_i$. Reranking then happens under CKKS in
the residual space, where the per-cell keys cancel,
$\langle H_c r_q, z_i\rangle=\langle r_q, r_i\rangle$, and the inner product
comes back \emph{exact}. The single knob $C$ interpolates between the
global-linear baseline it replaces ($C{=}1$) and per-document micro-keys
($C{=}N$), and along the way buys revocability at cell granularity.

What this construction buys---measured rather than assumed---comes to four
findings. The first is utility (Section~\ref{ssec:shard-utility}): because the
rerank is full-dimensional, \textsc{Shard} recovers the raw ranking that the
truncating baseline sacrifices, so that on the BEIR tasks where half-SVD
truncation significantly drops nDCG@10 it matches raw (e5-base/SciFact $0.638$
against raw $0.637$ and baseline $0.585$) while exposing a public channel only
a quarter to a half as wide. The second, and the headline, is that alignment
resistance scales with $C$ (Sections~\ref{ssec:shard-cost}--\ref{ssec:shard-align}):
under a \emph{diffuse} known-plaintext leak the anchors needed to map the
private residual back into its native frame grow roughly $C$-fold---a median
of $200$ for the global key, $25{,}600$ at $C{=}64$ and $102{,}400$ at
$C{=}256$, and the same on a second encoder---at a cost of some $7$--$30$
encrypted residual queries per search. We are careful to fence that claim in:
a \emph{targeted} attacker who concentrates anchors in a victim's public cell
needs only about $d_{\mathrm{priv}}$ of them ($320$ and $576$ on our two
encoders), so the $C\times$ gain is a property of diffuse, aggregate leakage
and not of every attacker. The barrier nonetheless survives the stronger
aligners we set against it---the learned-linear core of \textsc{ALGEN}, a
non-linear MLP, and unsupervised \textsc{vec2vec}-style matching all do no
better than the plain orthogonal Procrustes we use
(Section~\ref{ssec:shard-learned})---and against a distortion-aware DP-noise
defence held at matched utility, \textsc{Shard} alone reaches near-zero
de-anonymisation (Section~\ref{ssec:shard-vs-dp}).

The remaining two findings concern what the public side gives away and where
the guarantee stops. The short prefix leaks coarse rather than fine structure
(Sections~\ref{ssec:shard-leak},~\ref{ssec:shard-micro}): it reveals far less
neighbour structure than the baseline's exposed half (NN-overlap@10 climbing
from $0.20$ at $d_{\mathrm{pub}}{=}d/16$ to $0.55$ at $d/4$, against $0.76$),
the within-cell residual graph the server can rebuild shrinks with $C$ ($0.25$
at $C{=}64$, $0.18$ at $C{=}256$), and per-document micro-keys take it to
$0.00$ while leaving an unlinkable, renewable template (re-keying AUC
$\approx0.5$). The fourth finding we state rather than hide
(Section~\ref{ssec:shard-ref}): \textsc{Shard} hardens the diffuse
native-frame mapping, not the coarse graph, so within a cell the keys cancel,
and an \emph{overlapping} reference corpus still de-anonymises through the
public prefix once its one cheap global key is recovered. We measure that leak
and point to the coarse-cell-ID and micro-key variants that close it, each at
a recall or bandwidth cost.
...
paper contributes exactly the following new elements:
(i)~the \textsc{Shard} cell-keyed residual-splitting construction
(Section~\ref{sec:shard});
(ii)~the $\approx\!C\times$ anchor-complexity scaling law with a semi-formal
account (Proposition~\ref{prop:anchor}, Section~\ref{ssec:shard-align});
(iii)~the learned/non-linear/unsupervised aligner study against the keyed
residual and the learned-obfuscation and PQ-only defense baselines
(Sections~\ref{ssec:shard-learned},~\ref{ssec:shard-leak});
(iv)~the public-prefix leakage study and the per-document micro-key limit
(Sections~\ref{ssec:shard-leak},~\ref{ssec:shard-micro}); and
(v)~the targeted-vs-diffuse attacker distinction
(Section~\ref{ssec:shard-align}). Everything outside this list is reproduced
baseline context, condensed and attributed accordingly.

\paragraph{Hypotheses.} The study is organised around four testable
hypotheses. \textbf{(H1)} A single global keyed residual is recoverable
with $O(d)$ aligned anchors, whereas a cell-local keyed residual requires
$O(C\,d_{\mathrm{priv}})$ diffuse anchors. \textbf{(H2)} Full-dimensional
reranking over the keyed residual recovers raw retrieval quality more
faithfully than half-SVD truncation at a matched short-listing setup.
\textbf{(H3)} Public-stage leakage under the short prefix is lower than
under a public half-space PQ index at matched short-list utility.
\textbf{(H4)} Stronger learned aligners do not beat the
$d_{\mathrm{priv}}$ anchor threshold of a single cell. The experiments of
Section~\ref{sec:experiments} test each in turn.
...
against the studied attacker families}. Query values and scores are
protected cryptographically by CKKS; the document side is hardened, not
proven secure; and access patterns together with an overlapping public
reference corpus remain exposed channels that we measure rather than close.
...
lines on common axes: \textsc{Shard} is the only one combining cryptographic
query privacy, \emph{exact} full-dimensional reranking, and an
alignment-resistant coarse-only public channel, at the cost of $\approx C$
encrypted residual queries.
...
``exact rerank'' is full-dimensional scoring with no quantization/noise loss,
``coarse-only leak'' means the accessible channel exposes only coarse/topic
structure.}
...
\toprule
Defense & Query priv. & Doc.\ priv. & Exact rerank & Coarse-only leak & Attacker / cost \\ \midrule
\textbf{\textsc{Shard}} (ours) & $\checkmark$ CKKS & $\sim$ keyed & $\checkmark$ & $\checkmark$ & known-plaintext; $\approx C\times$ q. \\
...
DP-noise~\cite{ref_lyu} & $\times$ & $\sim$ noise & $\times$ & $\times$ & any (no key); low \\
\bottomrule
\end{tabular}
\end{table}
...
known-plaintext alignment attacker (diffuse, and for \textsc{Shard} also
targeted), public-index leakage, reference-corpus lookup, and, against
\textsc{Shard}'s keyed residual, the learned-linear (\textsc{ALGEN}) and
unsupervised (\textsc{vec2vec}) alignment cores together with a non-linear MLP
(Section~\ref{ssec:shard-learned}). Malicious-server behaviour and the full
\emph{generative} stages of the modern inverters stay out of scope.
...
alignment attack. \emph{Query privacy} is the one part that \emph{is}
cryptographic: CKKS hides the numerical value of the query vector and of the
similarity scores from the server, at the chosen security level. \emph{Access-pattern
...
No &
Appendices~\ref{ssec:vec2text}, \ref{ssec:adaptive-inversion} use
...
$v_q = E(q_{\mathrm{text}}) \in \mathbb{R}^d$, and forms the rotated query
$q' = R\, V_k^\top (v_q - \mu) \in \mathbb{R}^k$; using the public PQ artefact
and $\tilde q = q'$ it then runs an asymmetric PQ-distance search locally and
...
CKKS reranking still fits the latency budget. It encrypts the rotated query,
$\mathrm{ct}_q = \mathrm{Enc}_{pk}(q')$, and sends it on; for each
$j \in I_{\mathrm{cand}}$ the server computes
$\mathrm{ct}_{\mathrm{score}}^{(j)} = \mathrm{ct}_q \odot v'_j$ in ct-pt mode,
a ciphertext that encodes the inner product $\langle q', v'_j \rangle$ in the
rotated projected space. The client finally decrypts those scores,
...
A single rotated query $q' \in \mathbb{R}^{k}$ packs into one ciphertext over
...
The reproducible CKKS parameter selection recapped here is carried over from
our prior work~\cite{ref_vkit,ref_alpha} as part of the shared baseline
machinery---not a new contribution of this paper; we keep only the result
the ct-pt reranking needs. We are explicit about what does the work. The $\approx 1.7\times$ speed-up below is not a
...
The selected configuration is found by the offline grid search
of~\cite{ref_alpha}, recapped briefly. Over a discrete grid
($N_{\mathrm{poly}}\in\{2^{12},\dots,2^{14}\}$, security-valid modulus chains
bounded by the HomomorphicEncryption.org tables~\cite{ref_he_std}
cross-checked with the lattice estimator~\cite{ref_lwe_estimator}, and
several scales $\log_2\Delta$), a GradientBoosting latency
surrogate~\cite{ref_friedman} ($R^2{=}0.985$ on a held-out split of $198$
benchmarked configurations) returns the minimum-latency valid configuration
meeting an accuracy tolerance $|\Delta\mathrm{Acc@1}|\leq\tau$ against the
SVD baseline. With $\tau{=}1$~p.p.\ it selects $N_{\mathrm{poly}}{=}8192$,
$[60,40,60]$, $\Delta{=}2^{40}$ ($\log_2 Q{=}160$~bits, within the
conservative \texttt{tc128} bound), $\approx 1.7\times$ faster than the
TenSEAL stock $[60,40,40,60]$ ($\log_2 Q{=}200$) at the same security
(Table~\ref{tab:ckks-configs}).
...
\begin{proposition}[Exact cell-keyed reranking]\label{prop:exact}
...
\]
so $S(q,i)=\langle u_q,u_i\rangle+\langle r_q,r_i\rangle
=\langle x_q-\mu,\,x_i-\mu\rangle$ is reconstructed with no truncation.
...
the public-prefix term is unmodified, giving the stated full similarity. The
same-cell identity applies the same equality to the shared key $H_{c}$ of two
stored residuals.
\end{proof}
...
exact full-dimensional reranking, and the same-cell cancellation
$\langle z_i,z_j\rangle=\langle r_i,r_j\rangle$. Everything else below is
\emph{measured}, not guaranteed: the $\approx C\times$ known-plaintext
alignment cost (\S\ref{ssec:shard-align}), the resistance of that barrier to
learned and unsupervised attackers (\S\ref{ssec:shard-learned}), the
public-prefix leakage (\S\ref{ssec:shard-leak}), the micro-key limit
(\S\ref{ssec:shard-micro}) and the online latency
...
public prefix (\S\ref{ssec:shard-ref}). The barrier \textsc{Shard} raises is
the \emph{diffuse} alignment/inversion surface, not the coarse graph.
...
parameter selection that \textsc{Shard} reuses, the SVD-denoiser
significance and its non-transfer to BEIR, and the three failure modes
that motivate \textsc{Shard}: known-plaintext Procrustes alignment,
public-PQ leakage, and exact reference-corpus lookup. The
...
Appendix~\ref{ssec:adaptive-inversion} is the only non-local run: it uses
...
% ----------------------------------------------------------------
\subsection{Utility at $10^6$ scale: the denoiser effect and its transfer to BEIR}\label{ssec:projection-baselines}
% ----------------------------------------------------------------

We carry two utility properties of the global-linear baseline forward
only as context for \textsc{Shard}; both are reproduced from our prior
work~\cite{ref_alpha} and detailed, with their tables, in
Appendix~\ref{app:baseline}. The first is that, measured against lightweight
alternatives at $k=d/2$ (Gaussian and random-orthogonal
projection~\cite{ref_jl}), the choice of projection is an encoder-dependent
trade-off rather than a universal privacy primitive. The second is that a
calibrated Gaussian-noise diagnostic exposes the reconstruction proxy
$\sigma_{rec}$ as \emph{not} a privacy metric: matched-distortion noise can
preserve substantially more raw-neighbour structure than SVD at the same
$\sigma_{rec}$ (Appendix~\ref{app:noise}).
The remainder of this section establishes, at $10^6$ scale, whether
half-truncation acts as a denoiser, whether that effect is statistically
real, and whether it transfers to BEIR---prerequisites for the
\textsc{Shard} evaluation that follows.

% ----------------------------------------------------------------
\paragraph{Integral multi-encoder evaluation at $10^6$ scale.}\label{ssec:integral}
% ----------------------------------------------------------------

We run the full pipeline at the canonical operating point $k = d/2$ across
five encoders---\texttt{intfloat/multilingual-e5-small} ($d = 384$),
\texttt{intfloat/multilingual-e5-base} ($d = 768$),
\texttt{sentence-transformers/paraphrase-multilingual-mpnet-base-v2}
($d = 768$), \texttt{intfloat/multilingual-e5-large} ($d = 1024$) and
\texttt{BAAI/bge-m3} ($d = 1024$). The corpus is one million
Russian-Wikipedia paragraphs; queries are
...
% ----------------------------------------------------------------
\paragraph{Paired significance of the denoiser effect.}\label{ssec:significance}
% ----------------------------------------------------------------
...
The result sharpens the SVD-denoiser picture considerably. The Acc@1
``denoiser gains'' turn out not to be statistically significant for any
encoder---e5-base at $p=0.86$, e5-large at $p=0.11$, bge-m3 at $p=0.12$, with
all three bootstrap CIs straddling $0$---so the earlier reading that ``SVD
improves the four ranking metrics'' merely over-interpreted Acc@1 point
estimates. The effect that \emph{is} real and significant surfaces one cutoff
later, at Acc@10, and only for the retrieval-trained $d=1024$ encoders:
e5-large gains $+0.042$ ($p<10^{-6}$, and of the discordant queries $21$ are
pulled into the top-10 by SVD while none are lost), and bge-m3 $+0.016$
($p=0.04$). At the other end, for the compact e5-small and the
paraphrase-distilled mpnet, truncation significantly \emph{degrades} retrieval
at both cutoffs ($-0.082$ and $-0.064$ Acc@1, $p<10^{-5}$); and for e5-base the
projected space is statistically indistinguishable from raw at either cutoff,
so the wrapper is effectively free there. The corrected conclusion is thus
narrower and more defensible than ``SVD is a denoiser'': half-SVD truncation
is a mild, encoder-dependent denoiser that helps recall-style metrics for
retrieval-trained high-dimensional encoders and hurts compact or
paraphrase-distilled ones, but it does not reliably improve top-1 accuracy for
any encoder we tested.
...
% ----------------------------------------------------------------
\paragraph{Generality of the denoiser effect on BEIR.}\label{ssec:beir}
% ----------------------------------------------------------------
...
baseline's $k=d/2$. Even a $d/8$ prefix beats the baseline. On the $10^6$-document self-retrieval probe we add paired significance at the
same $d/4$ prefix. On e5-base \textsc{Shard} is statistically
indistinguishable from raw Acc@1 ($0.826$ vs.\ $0.830$; $-0.004$
$[-0.026,+0.018]$, McNemar $p{=}0.86$), and on e5-small it significantly
beats the SVD baseline ($0.734$ vs.\ $0.696$; $+0.038$ $[0.014,0.062]$,
$p{=}3{\times}10^{-3}$). A residual $\sim\!5$-point gap to raw remains on
e5-small at this short prefix ($p{=}7{\times}10^{-5}$)---a pure stage-1
short-list effect that closes at a wider $d/2$ prefix (Acc@1 $0.778$ vs.\ raw
$0.784$). Top-$1$ self-retrieval is stricter than top-$10$ nDCG, which is why
the $d/4$ prefix already recovers the raw nDCG@10 ceiling on BEIR while
top-$1$ keeps a small, prefix-tunable gap.
...
per-query nDCG@10 ($10^4$ resamples, seed $2026$). The SVD $k{=}d/2$
baseline reranks in the truncated half and loses nDCG; \textsc{Shard}
reranks full-dimensionally and recovers the raw ceiling with a public prefix
of only $d/4$.}
...
\begin{tabular}{@{}llrrrr@{}}
\toprule
Encoder & Dataset & raw & SVD $k{=}d/2$ $\Delta$\,[95\% CI] & \textsc{Shard} $d/4$ $\Delta$\,[95\% CI] & \textsc{Shard} $d/8$ $\Delta$\,[95\% CI]\\
\midrule
e5-base  & SciFact  & $0.637$ & $-0.052\,[{-}.073,{-}.032]$ & $\mathbf{+0.001\,[.000,.003]}$ & $-0.007\,[{-}.017,.000]$\\
e5-base  & NFCorpus & $0.327$ & $-0.022\,[{-}.033,{-}.013]$ & $\mathbf{-0.001\,[{-}.002,.000]}$ & $-0.005\,[{-}.008,{-}.001]$\\
e5-base  & ArguAna  & $0.346$ & $-0.082\,[{-}.091,{-}.073]$ & $\mathbf{-0.001\,[{-}.002,.001]}$ & $-0.005\,[{-}.008,{-}.002]$\\
e5-small & SciFact  & $0.598$ & $-0.076\,[{-}.100,{-}.053]$ & $\mathbf{-0.008\,[{-}.020,.001]}$ & $-0.025\,[{-}.042,{-}.009]$\\
e5-small & NFCorpus & $0.302$ & $-0.021\,[{-}.031,{-}.011]$ & $\mathbf{-0.002\,[{-}.004,.000]}$ & $-0.018\,[{-}.026,{-}.011]$\\
e5-small & ArguAna  & $0.358$ & $-0.060\,[{-}.069,{-}.051]$ & $\mathbf{-0.002\,[{-}.004,.000]}$ & $-0.013\,[{-}.017,{-}.008]$\\
\bottomrule
\end{tabular}
\end{table}
...
\noindent
Read as significance tests, the bootstrap CIs sharpen the claim: every
SVD-baseline $\Delta$ excludes $0$ (a significant $2$--$8$ nDCG-point loss),
whereas every \textsc{Shard}-$d/4$ $\Delta$ either includes $0$ or lies
within $\pm0.01$ of it (statistically indistinguishable from raw), while the
more aggressive $d/8$ prefix is small but mostly significant. Thus $d/4$ is
the operating point that recovers the raw ceiling---hypothesis~H2, confirmed
across three BEIR datasets and two encoders with per-query confidence
...
three-way ordering at $10^5$-document scale and on a different language
family per row: half-SVD truncation significantly loses nDCG@10 on every cell
($-0.021$ to $-0.065$; all CIs exclude $0$), while \textsc{Shard} with a $d/4$
prefix recovers the raw ceiling---exactly on e5-base ($\Delta=+0.000$) and
within $0.006$--$0.008$ on e5-small. The only departure from BEIR is the more
aggressive $d/8$ prefix on the compact e5-small encoder, which loses
$\approx\!0.10$ nDCG@10: a $48$-dimensional stage-1 short-list under-recalls
on a $10^5$-document corpus---an effect invisible on the
few-thousand-document BEIR corpora---whereas the $768$-dimensional e5-base is
unaffected. This reinforces $d/4$ as the operating point.
...
\begin{tabular}{@{}llrrrr@{}}
\toprule
Encoder & Language ($|C|$) & raw & SVD $k{=}d/2$ $\Delta$\,[95\% CI] & \textsc{Shard} $d/4$ $\Delta$\,[95\% CI] & \textsc{Shard} $d/8$ $\Delta$\,[95\% CI]\\
\midrule
e5-base  & Swahili ($132$k) & $0.707$ & $-0.021\,[{-}.034,{-}.009]$ & $\mathbf{+0.000\,[.000,.001]}$ & $-0.002\,[{-}.006,.002]$\\
e5-base  & Bengali ($297$k) & $0.727$ & $-0.030\,[{-}.042,{-}.019]$ & $\mathbf{+0.000\,[.000,.000]}$ & $-0.001\,[{-}.006,.002]$\\
e5-small & Swahili ($132$k) & $0.680$ & $-0.065\,[{-}.083,{-}.047]$ & $\mathbf{-0.006\,[{-}.015,.001]}$ & $-0.108\,[{-}.134,{-}.083]$\\
e5-small & Bengali ($297$k) & $0.688$ & $-0.051\,[{-}.067,{-}.035]$ & $\mathbf{-0.008\,[{-}.017,{-}.001]}$ & $-0.104\,[{-}.131,{-}.077]$\\
\bottomrule
\end{tabular}
\end{table}
...
We use $d_{\mathrm{pub}}=d/4$ ($d_{\mathrm{priv}}=288$ for e5-small, $576$
for e5-base), a $200\,000$-document pool and $5$ rotation seeds, and report
the median anchor count $m_{50}$ with a paired-bootstrap $95\%$ CI over
seeds${\times}$probes ($2000$ resamples), together with a \emph{random-cell}
control whose cells are assigned independently of the public prefix
(Table~\ref{tab:shard-align}, Fig.~\ref{fig:shard-align}). Before the
measurement, we state the scaling semi-formally.

\begin{proposition}[Diffuse anchor complexity]\label{prop:anchor}
Split the private residual into $C$ cells, each keyed by an independent
secret orthogonal $H_c\in\mathrm{O}(d_{\mathrm{priv}})$, and let a
known-plaintext attacker hold $m$ pairs $(r_i, H_{c(i)}r_i)$. Assume:
\emph{(A1)} recovering $H_c$ by orthogonal Procrustes needs
$\geq d_{\mathrm{priv}}$ anchors lying in cell $c$ in general position;
\emph{(A2, diffuse leak)} the anchors fall across cells with per-cell
fraction $p_c\leq(1+\delta)/C$ for a small $\delta\geq0$; \emph{(A3)} cells
are balanced up to that factor. Then de-anonymising a target in cell $c$
requires
\[
  m \;\geq\; \frac{d_{\mathrm{priv}}}{p_c}
    \;\geq\; \frac{C\,d_{\mathrm{priv}}}{1+\delta},
\]
so the diffuse anchor complexity is $\Theta(C\,d_{\mathrm{priv}})$---a factor
$\approx C$ above the single-key residual ($C{=}1$). A \emph{targeted}
attacker who concentrates anchors in the victim's (public) cell has $p_c{=}1$
and needs only $m\geq d_{\mathrm{priv}}$, independent of $C$.
\end{proposition}

\noindent\emph{Proof sketch.} The keys are independent across cells, so pairs
outside cell $c$ carry no information about $H_c$; orthogonal Procrustes then
fixes the $d_{\mathrm{priv}}{\times}d_{\mathrm{priv}}$ map uniquely once
$\geq d_{\mathrm{priv}}$ in-cell anchors are present (A1). The expected
in-cell count is $m\,p_c$; equating it to $d_{\mathrm{priv}}$ and bounding
$p_c$ by (A2)--(A3) gives the thresholds. The measured $m_{50}$ below match
$\Theta(C\,d_{\mathrm{priv}})$ up to the constant absorbed by ``general
position'' and cell imbalance. \hfill$\square$

\begin{table}[tbp]
\centering
\caption{Alignment anchor-complexity with a random-cell control and
paired-bootstrap $95\%$ CIs ($5$ seeds ${\times}$ $500$ probes, $2000$
resamples). $m_{50}$ is the known-plaintext anchor count at which residual
re-identification R@1 reaches $0.5$; k-means and random cells give the
\emph{same} $m_{50}$, so the resistance comes from cell-keying, not the
public-prefix clustering. $R$@$10^5$ is the k-means recovery rate at the full
$102{,}400$-anchor budget; $>\!10^5$ marks $m_{50}$ not reached within
budget.}
\label{tab:shard-align}
\small
\begin{tabular}{@{}llrrr@{}}
\toprule
Encoder & Scheme & $m_{50}$ (k-means) & $m_{50}$ (random) & $R$@$10^5$ [95\% CI] \\ \midrule
e5-small & global key & $200$ & --- & $1.00\,[1.00,1.00]$ \\
e5-small & \textsc{Shard} $C{=}64$ & $25\,600$ & $25\,600$ & $1.00\,[1.00,1.00]$ \\
e5-small & \textsc{Shard} $C{=}256$ & $102\,400$ & $102\,400$ & $0.93\,[0.92,0.94]$ \\ \midrule
e5-base & global key & $400$ & --- & $1.00\,[1.00,1.00]$ \\
e5-base & \textsc{Shard} $C{=}64$ & $51\,200$ & $51\,200$ & $1.00\,[1.00,1.00]$ \\
e5-base & \textsc{Shard} $C{=}256$ & $>\!10^5$ & $>\!10^5$ & $\mathbf{0.06\,[0.05,0.07]}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=\linewidth]{fig_shard_alignment.png}
\caption{Residual re-identification R@1 vs.\ known-plaintext anchors $m$, with
paired-bootstrap $95\%$ CI bands (left: e5-small, right: e5-base; $5$ seeds).
A single global key is recovered with $\sim d/2$ anchors; \textsc{Shard}
requires $\sim d_{\mathrm{priv}}$ anchors \emph{per cell}, shifting the curve
right by $\approx C$ (and off-scale for $C{=}256$ on e5-base). The micro-key
($C{=}N$) limit is unreachable: a per-document key cannot be recovered from
other documents' anchors.}
\label{fig:shard-align}
\end{figure}

The scaling is clean and close to linear in $C$: from $200$/$400$ anchors
for one global key (e5-small/e5-base) to $25\,600$/$51\,200$ at $C{=}64$ and
$102\,400$/${>}102\,400$ at $C{=}256$. The bootstrap makes three things
precise. First, the transition is sharp---Procrustes either has
$\ge d_{\mathrm{priv}}$ in-cell anchors and recovers the key exactly, or it
does not---so $m_{25}$, $m_{50}$ and $m_{75}$ coincide at our grid
resolution, and the recovery-rate CIs at the threshold are tight (e.g.\
e5-small $C{=}64$ reaches $0.91\,[0.90,0.92]$ at $25\,600$ anchors).
Second, and decisively for the baseline question, the \emph{random-cell}
control gives the same $m_{50}$ as k-means cells: the alignment resistance
is a property of cell-keying itself, not of the public-prefix clustering,
which only buys query locality (Section~\ref{ssec:shard-cost}). Third, the
strongest configuration here---$C{=}256$ on e5-base---holds the attacker to
$0.06\,[0.05,0.07]$ re-identification even at the full $102\,400$-anchor
budget, because with $d_{\mathrm{priv}}{=}576$ no cell receives its
$d_{\mathrm{priv}}$ in-cell anchors; this is hypothesis~H1, now measured with
confidence intervals and a random-cell control.

\paragraph{Baseline: a learned obfuscation.} Could a more \emph{complex}---
learned, nonlinear---global transform substitute for cell-keying? We test
the natural baseline: replace the orthogonal key with a single global MLP
$f$ trained to preserve inner products (retrieval), store $z=f(r)$, and let
the attacker train an inverse network $g$ on the $m$ known $(r,z)$ pairs. It
fails on both axes. The transform only partially preserves retrieval
(inner-product correlation $0.61$/$0.66$ for e5-small/e5-base, versus
\textsc{Shard}'s exact $1.0$) and, decisively, offers \emph{no} alignment
resistance: the learned inverse re-identifies over $90\%$ of targets from
only $m_{50}{=}72$ anchors on both encoders---\emph{below} even the global
orthogonal key's $200$, because a globally invertible map is learnable from
anchors anywhere. Together with the random-cell control, this isolates the
source of \textsc{Shard}'s $C\times$ resistance: not the complexity or
learnedness of the transform, but the \emph{locality of the secret}---a key
the attacker cannot fit from out-of-cell pairs.
...
% ----------------------------------------------------------------
\paragraph{Stronger and learned alignment attackers.}\label{ssec:shard-learned}
% ----------------------------------------------------------------
...
fails (sign ambiguity). Within the studied attacker family, no method
surpasses the $d_{\mathrm{priv}}$ single-cell anchor threshold.}
...
leak at a retrieval-quality cost; \textsc{Shard} preserves exact retrieval
...
e5-small & \textsc{Shard} $d/4$ prefix & exact ($1.000$) & $0.55$ \\
e5-small & PQ-only $M{=}24$ ($24$\,B) & $0.67$ & $0.27$ \\
e5-small & PQ-only $M{=}48$ ($48$\,B) & $0.83$ & $0.49$ \\ \midrule
e5-base & \textsc{Shard} $d/4$ prefix & exact ($1.000$) & $0.77$ \\
...
% ----------------------------------------------------------------
\paragraph{The micro-key limit: no residual leak, unlinkable.}\label{ssec:shard-micro}
% ----------------------------------------------------------------
...
gallery at a diffuse budget of $m{=}6400$ anchors, with paired-bootstrap
$95\%$ CIs over (seed${\times}$target).

The contrast is stark, and now carries CIs over a finer $\sigma$ sweep.
\emph{DP-noise provides essentially no resistance to this attack at any
usable utility}: there is no transform to invert, so the stored noised
residual still matches its own native entry---de-anon R@1 stays $1.000$
across the whole sweep and only dips to $0.996\,[0.993,0.999]$ at
$\sigma{=}2\sigma_r$, where utility has already fallen by six points. Read as
a matched-utility test this is decisive: DP equals \textsc{Shard}'s utility
($0.722$) only at $\sigma{=}0$, exactly where its de-anon is $1.000$; any
$\sigma$ that lowers de-anon strictly lowers utility below \textsc{Shard}'s.
A single global key is no better---it is recovered far below this budget
(Section~\ref{ssec:shard-align}). \textsc{Shard} alone reaches the top-left
corner: identical utility ($0.722$, the keyed rerank is exact) with de-anon
R@1 $=0.000$, because a diffuse budget of $6400$ anchors cannot fill any of
its cells. The mechanisms are complementary---one could compose
\textsc{Shard} with mild noise---but for the alignment/de-anonymisation
surface that the modern attacks target, keying is what pays.

\begin{table}[tbp]
\centering
\caption{Attack-aware vs.\ distortion-aware on the same residual (e5-small,
$d_{\mathrm{priv}}{=}288$). De-anon R@1 is a known-plaintext attacker at
$m{=}6400$ diffuse anchors, with paired-bootstrap $95\%$ CIs over
(seed${\times}$target). DP matches \textsc{Shard}'s utility ($0.722$) only at
$\sigma{=}0$, where de-anon is $1.000$; only \textsc{Shard} reaches high
utility \emph{and} low de-anonymisation.}
\label{tab:shard-vs-dp}
\small
\begin{tabular}{@{}lrl@{}}
\toprule
Defence (on the residual) & utility Acc@1 & de-anon R@1 [95\% CI] \\ \midrule
DP-noise $\sigma{=}0$ (no noise) & $0.722$ & $1.000\,[1.000,1.000]$ \\
DP-noise $\sigma{=}0.5\sigma_r$  & $0.719$ & $1.000\,[1.000,1.000]$ \\
DP-noise $\sigma{=}1.0\sigma_r$  & $0.711$ & $1.000\,[1.000,1.000]$ \\
DP-noise $\sigma{=}1.5\sigma_r$  & $0.694$ & $1.000\,[1.000,1.000]$ \\
DP-noise $\sigma{=}2.0\sigma_r$  & $0.666$ & $0.996\,[0.993,0.999]$ \\
\textbf{\textsc{Shard} $C{=}64$} & $\mathbf{0.722}$ & $\mathbf{0.000\,[0.000,0.000]}$ \\
\textbf{\textsc{Shard} $C{=}256$}& $\mathbf{0.722}$ & $\mathbf{0.000\,[0.000,0.000]}$ \\
\bottomrule
\end{tabular}
\end{table}
...
The most important caveat is that \textsc{Shard} hardens alignment, not the
coarse graph. The keyed residual resists native-frame mapping---$\approx
C\times$ anchors---but two channels stay open: within a cell the keys cancel,
so same-cell similarities still leak (about $0.50$ of the neighbour graph at
$C{=}64$), and the public prefix carries a single global key that
$d_{\mathrm{pub}}$ anchors recover, after which an \emph{overlapping} reference
corpus de-anonymises through it (Section~\ref{ssec:shard-ref}); the micro-key
and coarse-cell-ID variants would address both, at a bandwidth or recall cost
we have not yet paid. More broadly, \textsc{Shard} is a geometric rather than a
cryptographic defence---an attack-aware transform with measured advantages
against alignment and index leakage, but no formal privacy guarantee, and like
the baseline it leaves access patterns to a PIR/ORAM composition.

Two further limitations are inherited from the baseline it replaces. Document
privacy there is not cryptographic: $E_{\mathrm{rot}}$ sits in plaintext on the
server, protected only by SVD truncation (a proxy) and the secret rotation (an
empirical obfuscation layer), and Sections~\ref{ssec:alignment-pq}
and~\ref{ssec:reference-attack} show that known-plaintext anchors defeat the
rotation and that an overlapping reference corpus then recovers the exact
source paragraph. Access patterns and the public PQ codes are exposed as well:
CKKS hides values, not which candidates are reranked, and the PQ codes are a
lossy but non-negligible view of $E_{\mathrm{rot}}$, so production use needs a
PIR/ORAM layer together with a static-side protection for the index artefact.

The rest are matters of scope. The strict end-to-end budget
$\Delta\mathrm{Acc@1} \geq -5$~p.p.\ holds only on retrieval-trained encoders
with $d \geq 768$; compact or paraphrase-distilled encoders need either
replacement or a less aggressive $k$, which then breaks the
$\sigma_{rec} \geq 0.10$ proxy. Our largest experiment is a self-retrieval
geometry probe on $10^6$ Russian-Wikipedia paragraphs---a strong fidelity test
rather than a production benchmark---and although MIRACL
(Section~\ref{ssec:shard-utility}) now extends the graded-relevance evidence to
two languages at $\approx\!10^5$ documents each, the combination of $10^6$
scale \emph{and} graded relevance in a single corpus (the largest MIRACL
languages or MS~MARCO) stayed beyond our CPU budget. The generative-inversion
calibration is similarly corpus- and model-limited: the aligned Vec2Text stress
test runs on an offline synthetic-news+PII corpus with an off-the-shelf
corrector, and because raw inversion is already weak there, the zero-PII-recall
result reads as a boundary condition for that attacker rather than as general
document privacy. The latency figures of Section~\ref{ssec:latency}, finally,
are loopback measurements; a geographically distributed deployment will add
network latency we have not characterised.

\paragraph{Future work.}
The highest priority is to evaluate the stronger \textsc{Shard}
variants---the per-document micro-key and coarse-cell-ID channels that close the
within-cell and overlap-reference leaks of
Sections~\ref{ssec:shard-leak}--\ref{ssec:shard-ref}---measuring their
bandwidth (extra residual queries) and stage-1 recall costs, alongside a
learned per-cell key schedule. A second strand needs a GPU: Section~\ref{ssec:shard-learned}
stress-tests the \emph{alignment cores} of \textsc{ALGEN} (learned linear),
\textsc{vec2vec} (unsupervised) and a non-linear MLP and the barrier holds, but
the full generative stages---\textsc{ALGEN}'s text generator,
\textsc{ZSInvert}'s zero-shot LLM decoder, and a decoder fine-tuned on protected
embeddings---would confirm that failing to recover the native residual also
fails to recover text. The same logic applies to the baseline, where the
decisive text-level test is to fine-tune an inversion model on protected
embeddings from part of the corpus and report BLEU, token overlap and typed-PII
recovery on a disjoint holdout (Appendix~\ref{ssec:adaptive-inversion} only
evaluates an aligned off-the-shelf corrector), and to repeat the off-the-shelf
runs of Appendices~\ref{ssec:vec2text} and~\ref{ssec:adaptive-inversion} with
beam search, many more corrector iterations and a modern universal inverter so
that the ``BLEU at noise floor'' claim is not tied to one corrector.

On the calibration side, the $\sigma_{rec}=0.10$ proxy still rests on a
single-encoder, BLEU-only study---Appendix~\ref{app:noise} shows geometrically
that $\sigma_{rec}$ is not itself a privacy metric---so the required follow-up
is a multi-encoder ablation reporting BLEU, token overlap and typed-PII recovery
for names, addresses, e-mail, phone and medical terms. The lightweight-baseline
suite should likewise grow beyond the calibrated Gaussian-noise diagnostic,
which already covers SVD, Gaussian and random-orthogonal projection and
matched-distortion noise, to quantisation and noise mechanisms with formal DP
budgets at accuracy-matched operating points. For benchmarks, the gap that
remains is the $10^6$-scale combination of scale and graded relevance---the
largest MIRACL languages or MS~MARCO---together with the GPU-encoded
e5-large/NFCorpus cell that completes the grid.

The alignment study, finally, should be pushed past the overlap case of
Section~\ref{ssec:reference-attack} to the harder stress tests: non-overlapping
external corpora, semantic judges for near-neighbour leakage, and
embedding-space translation attacks such as vec2vec-style mappings. The
known-plaintext experiment already shows that \emph{linear} alignment can be
damaging; unsupervised or weakly supervised alignment against the keyed residual
is the open question.

Two further directions step outside \textsc{Shard}'s fixed-encoder setting and
tie it to a broader programme we pursue separately. The first concerns how wide
the public prefix should be: we set $d_{\mathrm{pub}}$ (and the truncation $k$)
to fixed fractions, but the right width is really a question of \emph{intrinsic
dimensionality}---how many dimensions a given language and encoder actually need
before the residual carries little but noise---so choosing $d_{\mathrm{pub}}$
and the cell count $C$ from the per-language SVD spectrum and
intrinsic-dimensionality estimates, rather than a flat $d/4$, should tighten
both the utility and the public-channel leak at once. The second is to
co-design the encoder instead of wrapping a frozen one: \textsc{Shard} protects
whatever geometry the encoder happens to ship, but an encoder trained for a
low-dimensional, inversion-resistant head---through head-aware contrastive
compression, or an adversarial low-dimensional shipped head---would make the
public prefix inherently harder to invert and would relieve the
encoder-dependence of the accuracy budget noted above, where compact encoders
presently force a replacement. Composing such a hardened head with
\textsc{Shard}'s cell-keyed residual is the natural endpoint: a defence that is
attack-aware both in the representation it ships and in the geometry it stores.
...
cell-keyed pieces, it recovers the raw-space ranking ceiling on the evaluated
cells---because reranking is full-dimensional it matches the raw nDCG@10 that
truncation drops on BEIR, and it does so while exposing a public channel only
half as wide. At the same time it multiplies the diffuse known-plaintext anchor
complexity of mapping the store back into the native frame by roughly $C$ (from
$200$ to $102{,}400$ anchors at $C{=}256$), a barrier that holds against
learned-linear, non-linear and unsupervised attackers alike and that a
matched-utility DP-noise defence does not approach. And it confines the public
index to coarse, topic-level neighbours while localising the residual
neighbour-graph leak to cells---both shrinking with $C$ and vanishing at the
per-document micro-key limit. A single parameter $C$ tunes the family from the global baseline
...
\appendix
% ----------------------------------------------------------------
\section{Reproduced baseline analyses from the global-linear system}\label{app:baseline}
% ----------------------------------------------------------------

The analyses in this appendix characterise the \emph{global-linear
baseline} (SVD truncation $+$ secret rotation $+$ CKKS reranking), not
\textsc{Shard}. They are reproduced in condensed form from our prior
work~\cite{ref_vkit,ref_alpha}, where the inversion-versus-rotation study
is the primary contribution; we include the evidence tables here so this
paper is self-contained, and refer the reader to~\cite{ref_alpha} for the
full treatment. \textsc{Shard}'s own evaluation is in
Section~\ref{sec:experiments}.

\subsection{Off-the-shelf Vec2Text against truncation and rotation}\label{ssec:vec2text}
We instantiate \textsc{Vec2Text}~\cite{ref_vec2text} on \textsc{GTR-base}
embeddings ($d=768$) over a synthetic $100$-document PII corpus, under
three threat models---no rotation; rotation with $R$ \emph{known};
rotation with $R$ \emph{unknown}---at five values of $k/d$. The attacker
budget is fixed and deliberately weak (greedy decoding, $10$ corrector
iterations, no beam search), so the unprotected BLEU ($0.156$) is far
below the $0.973$ of~\cite{ref_vec2text}; the experiment measures the
\emph{relative} effect of truncation and rotation, not absolute inversion
risk. This is the single most important caveat of the baseline's security
narrative: an unknown $R$ drops BLEU to the noise floor ($\sim0.007$) for
every $k/d$ (Table~\ref{tab:vec2text}), but only against this
non-adaptive inverter---a known $R$ gives no protection beyond
truncation, and the known-plaintext alignment of
Section~\ref{ssec:alignment-pq} defeats the rotation outright. A linear
fit of the no-rotation BLEU on $\eta_k=1-\sigma_{rec}^2$ gives
$R^2\approx0.93$, consistent with Hypothesis~\ref{hyp:bleu}.
...
\subsection{Lightweight projection baselines}\label{app:projbaselines}
To avoid comparing the transform only against the raw system, we evaluate
SVD against Gaussian and random-orthogonal projection~\cite{ref_jl} at
$k=d/2$ (Table~\ref{tab:projection-baselines}). The result is
deliberately mixed: SVD is strongest on the retrieval-trained $d\geq768$
encoders---the operating region of the method---but random-orthogonal
projection beats it on mpnet and e5-small. The choice of projection is an
encoder-dependent utility trade-off, not a universal privacy primitive.
...
\subsection{Calibrated Gaussian-noise diagnostic}\label{app:noise}
On a $100\,000$-document gallery (PCA basis from an $80\,000$ sample) we
sweep $k/d\in\{1/8,1/4,1/2,7/8\}$ and, for each $k$, add isotropic
Gaussian noise calibrated by binary search to the SVD $\sigma_{rec}$,
reporting self-retrieval Acc@1 and exact top-$10$ raw-vs-protected
nearest-neighbour overlap (Table~\ref{tab:noise-sweep}). Two readings
follow. First, $\sigma_{rec}$ is \emph{not} a privacy metric: e5-base at
$k/d=0.875$ has low distortion yet preserves almost all raw neighbours
under matched noise. Second, SVD changes geometry differently from
independent noise---at $k/d=0.5$ it preserves far fewer raw neighbours
($0.578$ vs.\ $0.829$ NN@10 on e5-base) without the ranking collapse of
very aggressive truncation. The narrow claim is that SVD is an
encoder-dependent utility/leakage trade-off, not that noise is simply
worse.
...
As a portable generative-inversion control we run the off-the-shelf
\textsc{Vec2Text} corrector with a larger budget than above ($24$
corrector iterations, max input length~$96$) against GTR-base embeddings
($d=768$, $k=d/2$, secret rotation), given up to $m=500$ known
plaintext/protected pairs for Procrustes alignment, on a deterministic
offline synthetic-news$+$PII corpus ($2\,200$ texts, $120$ test, $13$
with phone/card patterns). No exact document or typed PII is recovered in
any case---including a known-$R$ oracle---but the \emph{raw} row is also
at the floor (BLEU $0.013$, Table~\ref{tab:adaptive-inversion}), so this
off-the-shelf corrector certifies nothing. It does not contradict the
exact-lookup failure of Section~\ref{ssec:reference-attack}; the open
risk is a learned, corpus-adapted decoder (Section~\ref{sec:limits}).
...
\texttt{BeIR/scifact} and \texttt{BeIR/nfcorpus} releases. The code,
configuration and per-experiment outputs are released at
\url{https://github.com/sergkurilenko/research/tree/article5}.

% =====================================================================
\section*{Declarations}
% =====================================================================

\paragraph{Ethics.} This study used public benchmark corpora (Wikipedia,
BEIR) and synthetic privacy-stress examples only; no intervention involving
human participants was conducted and no non-public personal data were
collected. Institutional ethics review was therefore not required under our
institution's policy.

\paragraph{Publication overlap.} Part of the baseline description---the
global-linear SVD$+$rotation$+$CKKS pipeline and the CKKS parameter
selection---reproduces our previously published system~\cite{ref_vkit} and
its companion preprint~\cite{ref_alpha}, and is included here only as the
comparison setting against which \textsc{Shard} is measured. The new
contributions of this paper do not overlap with that earlier work: the
\textsc{Shard} cell-keyed construction; the additional attacker families
(learned/non-linear/unsupervised aligners, targeted-cell, reference-corpus);
the anchor-complexity, public-prefix-leakage and micro-key measurements; and
the evaluation beyond the prior paper. This manuscript is not under
